...
BONUS_THRESHOLDS = [
    ...
    (200, "mines"),         # sblocca MINES_COUNT mine sganciabili sulla mappa (si attivano col tasto "1")
    (400, "missile"),       # sblocca 1 missile guidato (si spara col tasto "3")
    (600, "turret"),        # sblocca la torretta automatica piazzabile (si piazza col tasto "5")
]
...
MINES_COUNT = 2                # numero di mine disponibili una volta sbloccato il bonus 200 punti
...
# I portali non sono piu' sempre attivi: si accendono e si spengono a
# ciclo, PORTAL_ACTIVE_SECONDS acceso seguiti da PORTAL_ACTIVE_SECONDS
# spento (stesso valore per entrambe le fasi, come richiesto: "si accendano
# per 30 secondi con ogni intervallo di 30 secondi"). Da spenti non
# teletrasportano piu' nessuno finche' non tornano accesi.
PORTAL_ACTIVE_SECONDS = 30.0
...
# da sola e l'avversario torna libero. La trappola si puo' usare al
# massimo TRAP_USES_COUNT volte per giocatore in un round (la detonazione
# di una trappola gia' piazzata non consuma un altro uso).
TRAP_THRESHOLD = 500
TRAP_DURATION_SECONDS = 3.0
...
# in quel momento. La torretta e' permanente (resta per tutto il resto del
# round, non si esaurisce e non puo' essere ripiazzata) e spara da sola,
# alla stessa cadenza del laser (TURRET_FIRE_INTERVAL_SECONDS ==
# LASER_INTERVAL_SECONDS), verso il nemico vivo piu' vicino in quel
# momento, riusando la stessa fisica dei proiettili laser.
TURRET_FIRE_INTERVAL_SECONDS = LASER_INTERVAL_SECONDS
