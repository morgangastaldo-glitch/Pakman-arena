...
from common import (
    ...
    PORTAL_COOLDOWN_SECONDS, PORTAL_ACTIVE_SECONDS,
    MISSILE_SPEED_MULT, MISSILES_COUNT, MISSILE_RETARGET_SECONDS,
    TRAP_THRESHOLD, TRAP_DURATION_SECONDS, TRAP_RANGE, TRAP_USES_COUNT,
...
class Player:
    def __init__(self, pid, name, ws):
        ...
        self.trap_uses_left = 0        # quante volte puo' ancora intrappolare qualcuno (max TRAP_USES_COUNT)
        # ---- bonus 600 punti: torretta automatica (tasto "5") ----
        self.has_turret = False        # bonus sbloccato (una volta per round)
        self.turret_placed = False     # True dopo il piazzamento: non se ne puo' piazzare un'altra
...
class Room:
    ...
    def pick_new_map(self):
        ...
        # I portali partono sempre accesi a inizio round, poi si alternano
        # a ciclo (vedi update_portals): PORTAL_ACTIVE_SECONDS accesi,
        # PORTAL_ACTIVE_SECONDS spenti, e cosi' via.
        self.portal_active = True
        self.portal_phase_left = PORTAL_ACTIVE_SECONDS
    ...
    def check_bonuses(self, p):
        ...
        if (
            ...
            p.has_trap = True
            p.trap_uses_left = TRAP_USES_COUNT
            self.push_event({
                "kind": "bonus", "player": p.id,
    ...
    def update_portals(self):
        """Fa alternare i portali tra acceso e spento: PORTAL_ACTIVE_SECONDS
        accesi, poi PORTAL_ACTIVE_SECONDS spenti, a ciclo continuo per tutto
        il round. Da spenti non teletrasportano piu' nessuno (vedi
        try_portal)."""
        if not self.portals:
            return
        self.portal_phase_left -= TICK_DT
        if self.portal_phase_left <= 0:
            self.portal_active = not self.portal_active
            self.portal_phase_left = PORTAL_ACTIVE_SECONDS
            self.push_event({"kind": "portal_toggle", "active": self.portal_active})

    def try_portal(self, p):
        """Se il giocatore e' su un portale acceso (e non e' appena arrivato
        da un teletrasporto), lo sposta al portale opposto mantenendo la
        direzione. Da spenti (vedi update_portals) non fanno nulla."""
        if not self.portals or p.portal_cd > 0 or not self.portal_active:
            return
        pos = (p.x, p.y)
    ...
    def spawn_laser(self, shooter):
        ...
        self.spawn_laser_projectile(shooter.id, shooter.x, shooter.y, dx, dy, facing=shooter.facing)

    def spawn_laser_projectile(self, owner_id, x, y, dx, dy, facing=None):
        """Versione generica di spawn_laser: crea un proiettile laser da un
        punto e una direzione qualsiasi, non necessariamente quelli di un
        giocatore. Usata sia dal laser dei giocatori (bonus 150 punti) sia
        dalla torretta automatica (bonus 600 punti, stessa cadenza di fuoco
        e stessa fisica di volo/collisione: vedi move_lasers)."""
        laser = {
            "id": uuid.uuid4().hex[:8],
            "owner": owner_id,
            "x": x, "y": y,   # cella intera corrente
            "dx": dx, "dy": dy,
            "move_accum": 0.0,
        ...
        self.push_event({
            "kind": "laser_fire", "id": laser["id"], "shooter": owner_id,
            "x": x, "y": y, "dir": facing,
        })
        return laser
    ...
    def try_activate_trap(self, player):
        ...
        - Se questo giocatore non ha ancora nessuno intrappolato (o la sua
          ...
          nemico piu' vicino: resta bloccato sul posto per soli
          TRAP_DURATION_SECONDS (3 secondi).
        ...
        Puo' intrappolare al massimo TRAP_USES_COUNT (3) volte per round:
        ogni nuovo intrappolamento consuma un uso, la detonazione di una
        trappola gia' piazzata no."""
    ...
    def try_place_turret(self, player):
        ...
        torretta automatica nella cella in cui si trova il giocatore in
        questo istante. E' permanente (nessuna durata, nessuna scorta) e da
        quel momento spara da sola (vedi update_turrets), quindi dopo il
        piazzamento il bonus non e' piu' riattivabile."""
        if not player.alive or not player.has_turret or player.turret_placed:
            return
        if any(t["x"] == player.x and t["y"] == player.y for t in self.turrets):
            return  # niente due torrette sulla stessa cella
        player.turret_placed = True
        turret = {
            "id": uuid.uuid4().hex[:8], "owner": player.id,
            "x": player.x, "y": player.y,
            "cd": TURRET_FIRE_INTERVAL_SECONDS,
        }
    ...
    def update_turrets(self):
        """Ogni torretta piazzata spara da sola, alla stessa cadenza del
        laser (TURRET_FIRE_INTERVAL_SECONDS == LASER_INTERVAL_SECONDS),
        verso il nemico vivo piu' vicino in quel momento (proprietario
        escluso). Il colpo riusa esattamente la stessa fisica dei proiettili
        laser (vedi move_lasers): si estingue sul primo muro o sulla prima
        vittima colpita."""
        if not self.turrets:
            return
        for t in self.turrets:
            ...
            if dx == 0 and dy == 0:
                continue
            # Mira solo sui 4 assi cardinali (come il laser): sceglie
            # l'asse con la distanza maggiore verso il bersaglio.
            if abs(dx) >= abs(dy):
                fdx, fdy = (1 if dx > 0 else -1), 0
            else:
                fdx, fdy = 0, (1 if dy > 0 else -1)
            self.spawn_laser_projectile(t["owner"], t["x"], t["y"], fdx, fdy)
    ...
    async def run_round(self):
        ...
        while self.state in ("COUNTDOWN", "PLAYING"):
            ...
            self.update_portals()  # accensione/spegnimento ciclico (30s / 30s)
            ...
            if self.state == "COUNTDOWN":
                ...
                self.update_turrets() # bonus 600 punti: le torrette piazzate sparano da sole (stessa cadenza)
                self.move_lasers()    # avanza i proiettili laser in volo (anche quelli appena sparati)
    ...
    try:
        async for raw in ws:
            ...
            if mtype == "create_room":
                ...
                # Bonus 600 punti: pressione del tasto "5" lato client, una
                # sola volta per round (la torretta piazzata e' permanente).
                if not room or not player:
                    continue
